function numeric()
    ...
    path = full_path(1:end-14);
    for i=1:data.numbers
        data.images{i} = imread([path image_set filesep num2str(i) '.jpg']);
    end;
    ...
    uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'Position', [init_x-200 init_y 250 30],...
                        'String', 'Your Score:', ...
    ...
    data.score = uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x-45 init_y 160 30],...
                        'String', init_score, ...
                        'FontSize', fontSize);
    ...
    uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+30 init_y 250 30],...
    ...
    data.show_timer = uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+230 init_y 160 30],...
                        'String', data.timer, ...
                        'FontSize', fontSize);
    ...
    uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+280 init_y 250 30],...
                        'String', 'Gaming time:', ...
                        'FontWeight', fontWeight,...
                        'FontSize', fontSize);
   
                    
    data.show_clock = uicontrol(Figure_h, 'Style', 'text', ...
                        'BackgroundColor', backgroundColor, ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+450 init_y 160 30],...
                        'String', data.clock, ...
                        'FontSize', fontSize);
    ...
    uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+530 init_y 250 30],...
                        'String', 'Your Level:', ...
                        'FontWeight', fontWeight,...
                        'FontSize', fontSize);
    ...
    data.level = uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', foregroundColor, ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+680 init_y 160 30],...
                        'String', cur_level, ...
                        'FontSize', fontSize);

    matrix = randi(data.numbers, data.mat_size);
    y = init_y - 70 -(10-size(matrix,1))*22.5;

    for i = 1:size(matrix,1)
        x = init_x + (10-size(matrix,1))*22.5;
        for j = 1:size(matrix,2)
            data.elements(i,j) = uicontrol(Figure_h, 'Style', 'pushbutton', 'Position', [x y 50 50], 'String', '', 'BackgroundColor', 'green', 'FontSize', fontSize, 'Callback', {@select_it, i,j});
            uib = data.elements(i,j);
            change_color(uib, data.images{abs(matrix(i,j))}, 0, -1, 0);
            x = x+50; 
        end
        y = y-50;
    end
    ...
    data.new_level = uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', 'red', ...
                        'HorizontalAlignment', 'center',...
                        'Position', [init_x-50 235 500 130],...
                        'String', 'Level:', ...
                        'FontWeight', fontWeight,...
                        'FontSize', 80);
    data.new_level_num = uicontrol(Figure_h, 'Style', 'text', ...
                        ...
                        'ForegroundColor', 'red', ...
                        'HorizontalAlignment', 'left',...
                        'Position', [init_x+370 235 300 130],...
                        'String', cur_level, ...
                        'FontWeight', fontWeight,...
                        'FontSize', 80);
...
function run_numeric(src, evnt, fig)
    ...
    data.clock_obj = timer('StartDelay', 1, 'ExecutionMode', 'fixedRate');

    
    data.timer_obj.TimerFcn = {@use_timer ,fig}; 
    data.clock_obj.TimerFcn = {@use_clock ,fig}; 
    start(data.clock_obj);
...
function use_clock(obj, evnt, src)
    data = guidata(src);
    set(data.show_clock, 'String', str2num(get(data.show_clock, 'String'))+1);
    guidata(src, data);
    
end

function change_color(obj, img, R, G, B)
    if(R>=0)
        img(:,:,1) = R;
    end;
    if(G>=0)
    ...
    imfit = imresize(img, [pos(end) pos(end-1)]);
    set(obj, 'Cdata', imfit);
        ...
        while length >= data.min_string
            ...
            if(size(first_ltr,1)>0)
                for js = first_ltr
                    ...
                    fileID = fopen('tmp.txt','a');
                    fprintf(fileID, '%s: add_score: %d\n', datestr(now), add_score);
                    fclose(fileID);

                    data.timer = min(data.timer + data.timer_increase, data.timer_max);
                    
                   
                    for k=js:js+length-1
                        uib = data.elements(i,k);
                        if(same)
                            change_color(uib, data.images{abs(data.matrix(i,k))}, 150, -1, -1);
                        else
                            change_color(uib, data.images{abs(data.matrix(i,k))}, 0, -1, 256);
                        end;
                    end
                    
                    data.matrix(i,js:js+length-1) = -data.matrix(i,js:js+length-1);
                    data.changed = true;
                end
            end;
            length = length -1;
        ...
        while length >= data.min_string
            ...
            if(size(first_ltr, 1)>0)
                for js = first_ltr
                    ...
                    for k=js:js+length-1
                        uib = data.elements(k,j);
                        if(same)
                            change_color(uib, data.images{abs(data.matrix(k,j))}, 150, -1, -1);
                        else
                            change_color(uib, data.images{abs(data.matrix(k,j))}, 0, -1, 256);
                        end;
                                       
                    end  
                    data.matrix(js:js+length-1, j) = -data.matrix(js:js+length-1, j);
...
function select_it(src, evnt, i, j) %read text data files and convert them to mat files
    ...
    if(data.selected(i,j) ==1)
        change_color(uib, data.images{abs(data.matrix(i,j))}, 0, -1, 0);
        data.selected(i,j) = 0;
        guidata(src,data);
        return;
    end;
    ...
    uib_prev = data.elements(prev_i, prev_j);
    set(data.elements(i,j),'BackgroundColor','red');        
    change_color(uib, data.images{abs(data.matrix(i,j))}, -1, 0, 0);

    data.selected(i, j) = 1;
    if size(prev_i,1)>0 && ((abs(prev_i-i)==1 && abs(prev_j-j)==0) || (abs(prev_i-i)==0 && abs(prev_j-j)==1))  

        change_color(uib, data.images{abs(data.matrix(i,j))}, -1, 0, -1);
        change_color(uib_prev, data.images{abs(data.matrix(prev_i, prev_j))}, -1, 0, -1);
...
%         set(data.elements(i, j), 'String', data.matrix(i, j));
    ...
    if(uib_prev)
        change_color(uib_prev, data.images{abs(data.matrix(prev_i, prev_j))}, 0, -1, 0);
    end;
    data.selected(prev_i, prev_j) = 0;
    guidata(src,data);
    if(clean_it)
